...
// Replace ROOM_IMAGES with local images (correct paths)
const ROOM_IMAGES = {
...
// SVG path for bedroom (converted from provided polyline)
...
// SVG path for living room (from revised SVG)
...
// SVG path for kitchen (from revised SVG)
...
// Update WALL_MASKS to support multiple sides per room (placeholder paths)
...
const BasicVisualiserPage: React.FC = () => {
  ...
  // For mobile swatch bar intersection logic
  const [swatchFixed, setSwatchFixed] = useState(true);
      ...
      <main className="min-h-screen bg-gray-50 pt-4 md:pt-16 pb-24 flex flex-col items-center px-4 md:px-0">
        ...
        <div className="flex flex-col md:flex-row gap-10 w-full max-w-screen-xl">
          ...
          <div ref={imagesContainerRef} className="flex-1 flex flex-col gap-8 relative">
            ...
            {/* Sentinel for intersection observer (after images, before swatch bar) */}
            <div ref={bottomSentinelRef} className="block md:hidden w-full h-1" />
            {/* Swatch bar for mobile/tablet: fixed above buttons while images in view, static after */}
            <div
