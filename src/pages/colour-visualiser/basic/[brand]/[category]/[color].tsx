...
const BasicVisualiserPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (typeof colorParam === 'string') {
      const lastDash = colorParam.lastIndexOf('-');
      if (lastDash !== -1) {
        const name = fromKebabCase(colorParam.substring(0, lastDash));
        const code = colorParam.substring(lastDash + 1).replace(/-/g, ' ');
        colorObj = colors.find((c: any) => toKebabCase(c.colorName) === toKebabCase(name) && c.colorCode === code);
      }
    }
      ...
      <main className="min-h-screen bg-gray-50 pt-4 md:pt-16 pb-2 flex flex-col items-center px-4 md:px-0">
        ...
        {colorDatabase && (
          <div className="w-full flex justify-center mb-4">
            <div className="flex gap-2 w-[90%] overflow-x-auto flex-nowrap scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent justify-center">
              {Object.keys(colorDatabase.colorTypes).map((cat: string) => {
                const categoryColor = CATEGORY_COLORS[cat] || '#ED276E';
                return (
                  <button
                    key={cat}
                    className={`px-4 py-1 rounded-full font-medium border transition-all duration-200 whitespace-nowrap text-sm flex-shrink-0 ${
                      selectedCategory === cat 
                        ? 'text-white border-current' 
                        : 'bg-white text-current border-current hover:bg-opacity-10'
                    }`}
                    style={{
                      backgroundColor: selectedCategory === cat ? 'white' : categoryColor,
                      color: selectedCategory === cat ? categoryColor : 'white',
